			...
			while (reader.hasNext() && !connection.isClosed()) {
				...
				if (reader.getEventType() == XMLStreamReader.START_ELEMENT) {
					...
					if (session == null)
						...
						System.out.println("received (" + session.getJID().getUserName() + "): " + currentStanza.getName());
					for (int i = 0; i < reader.getAttributeCount(); i++) {
					...
					if (isCurrent("{http://etherx.jabber.org/streams}stream")) {
						...
						if (currentStanza.getAttributes().containsKey("to") && currentStanza.getAttributes().containsKey("version")) {
							if (stream == null) {
								// not ssl
								if (!connection.isSSL() || !new File(connection.getKeyStoreFileName()).exists()) {
									stream = new Stream(currentStanza.getAttributes().get("to"), currentStanza.getAttributes().get("version"));
					...
					if (isCurrent("{urn:ietf:params:xml:ns:xmpp-sasl}auth")) {
						if (currentStanza.getAttributes().containsKey("mechanism")) {
							authenfication = new SASLAuthenfication(connection, stream, currentStanza.getAttributes().get("mechanism"));
						} else {
							connection.close();
							break;
						}
					...
					if (isCurrent("{http://jabber.org/protocol/disco#info}query")) {
						if (isIqGet(1)) {
							connection.sendString("<iq type='result' from='" + stream.getServer() + "' " + "id='"
									+ stanzaStack.get(1).getAttributes().get("id") + "'>" + "<query xmlns='http://jabber.org/protocol/disco#info'>"
									+ "<identity " + "category='conference' " + "type='text' " + "name='Play-Specific Chatrooms'/>" + "<identity "
					...
					if (isCurrent("{jabber:iq:roster}query")) {
						if (isIqGet(1)) {
							...
							String answer = "<iq type='result' from='" + stream.getServer() + "' " + "id='" + stanzaStack.get(1).getAttributes().get("id")
									+ "'>";
							if (!items.isEmpty()) {
								answer += "<query xmlns='jabber:iq:roster'>";
								for (JID contact : items)
									answer += "<item jid='" + contact.getJIDString() + "' name='" + contact.getNickName() + "' subscription='"
					...
					if (isCurrent("{jabber:client}presence") && getParameterAt(0, "type") != null) {
						if (getParameterAt(0, "type").equals("unsubscribe")) {
							...
							if (to != null) {
								...
								XMPPServer.getDatabase().setSubscriptionLevel(jID, session.getJID().getUserName(), "none");
							...
							if (to != null) {
								...
								if (!jID.getAskLevel().equals("subscribe")) {
									...
									XMPPServer.getDatabase().setSubscriptionLevel(jID, session.getJID().getUserName(), "to");
						...
						if (getParameterAt(0, "type").equals("subscribed")) {
							...
							if (to != null) {
								...
								if (jID.getSubscriptionLevel().equals("none") && !jID.getAskLevel().equals("subscribe")) {
									...
									XMPPServer.getDatabase().setSubscriptionLevel(jID, session.getJID().getUserName(), "from");
									session.sendPresenceTo(jID, null, null, null, "subscribed");
									session.sendSubscriptionRoster(jID);
								} else

								if ((jID.getSubscriptionLevel().equals("none") || jID.getSubscriptionLevel().equals("from") || jID.getSubscriptionLevel()
										.equals("to")) && jID.getAskLevel().equals("subscribe")) {
									JID myJid = new JID(session.getJID().getJIDString());

									jID.setSubscriptionLevel("both");
									XMPPServer.getDatabase().setSubscriptionLevel(jID, myJid.getUserName(), "both");

									session.getJID().setSubscriptionLevel("both");
									XMPPServer.getDatabase().setSubscriptionLevel(myJid, jID.getUserName(), "both");
						...
						if (session.getJID().getGroup() != null)
							if (session.getJID().getGroup().equals("root")) {
								...
								if (command.startsWith("kick")) {
									...
									for (String user : users) {
										Session scriptSession = connection.getOpenSessions().get(user + "@" + stream.getServer());
